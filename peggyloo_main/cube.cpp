...
float Dx=.1,Dy=.13,Dz=0;
float zCamera = 110; // distance from cube to the eye of the camera
...
struct Cube
{
  ...
  void draw()
  {
    ...
    for (i=0;i<12;i++)
    {
      e = edge+i;
      if (!e->visible)
        frame.Line(1, screen[e->x].x,screen[e->x].y,screen[e->y].x,screen[e->y].y);
    }
    ...
    for (i=0;i<12;i++)
    {
      ...
      if (e->visible)
      {
        frame.Line(3, screen[e->x].x,screen[e->x].y,screen[e->y].x,screen[e->y].y);
      }
    }
...
boolean cube_loop(boolean skip)
{
  // Update values
  Angx+=AngxSpeed;
  Angy+=AngySpeed;
  ...
  {
    frame.Refresh(0,1); //Draw frame buffer 1 time
    frame.Refresh(1,10); //Draw frame buffer 10 times
  }
#endif
  ...
  if (!frame.Button()) {
	  ...
	  if ((Ox > 16) || (Ox < 9)) { Dx = -Dx; };
	  if ((Oy > 24) || (Oy < 9)) { Dy = -Dy; };
	  if ((zCamera < 110) || (zCamera > 220)) { Dz = -Dz; };
  }
